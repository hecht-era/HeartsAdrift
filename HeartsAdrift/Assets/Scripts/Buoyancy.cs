...
public class Buoyancy : MonoBehaviour
{
	...
	private void Start()
	{
		...
		if (GetComponent<Collider>() == null)
		{
			...
			Debug.LogWarning(string.Format("[Buoyancy.cs] Object \"{0}\" had no collider. BoxCollider has been added.", name));
		}
		isBoxCollider = GetComponent<BoxCollider>() != null;
		...
		voxels = SliceIntoVoxels(isBoxCollider && isConcave);
			...
			for (int ix = 0; ix < slicesPerAxis; ix++)
			{
				for (int iy = 0; iy < slicesPerAxis; iy++)
				{
					for (int iz = 0; iz < slicesPerAxis; iz++)
					{
						...
						if (PointIsInsideBoxCollider(meshCol, p))
						{
							points.Add(p);
						}
					}
	...
	private static bool PointIsInsideBoxCollider(Collider c, Vector3 p)
	{
