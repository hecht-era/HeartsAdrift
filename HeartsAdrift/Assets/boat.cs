...
public class boat : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Physics.Raycast(ray, out hit, 100f, LayerMask.GetMask("Interactable")))
        {
            ...
            else if (hit.collider.gameObject.transform.tag == "Map")
            {
        ...
        if (StateManager.Instance.GetState() == GameState.READING)
        {
            if(_hitBook != null)
            {
                ...
                book.transform.position = Vector3.MoveTowards(book.transform.position, camFront.transform.position, 1f * Time.deltaTime);
                //book.transform.rotation = Quaternion.LookRotation(cam.transform.position, book.transform.up);
                book.transform.localScale = Vector3.MoveTowards(book.transform.localScale, new Vector3(0.9362202f, 0.2698442f, 1.182919f), 1f * Time.deltaTime);
            ...
            if(_hitMap != null)
            {
                ...
                map.transform.position = Vector3.MoveTowards(map.transform.position, camFront.transform.position, 1f * Time.deltaTime);
                //map.transform.rotation = Quaternion.LookRotation(cam.transform.position, map.transform.up);
                map.transform.localScale = Vector3.MoveTowards(map.transform.localScale, new Vector3(0.9362202f, 0.2698442f, 1.182919f), 1f * Time.deltaTime);
                if (Input.GetKeyDown(KeyCode.R))
                {
                    StateManager.Instance.SetState(_lastState);
                    _lastState = GameState.READING;
        ...
        if(_lastState == GameState.READING)
        {
            ...
            //book.transform.rotation = Quaternion.LookRotation(cam.transform.position, book.transform.up);
            book.transform.localScale = Vector3.MoveTowards(book.transform.localScale, new Vector3(0.2923884f, 0.08427431f, 0.3694341f), 1f * Time.deltaTime);

            map.transform.position = Vector3.MoveTowards(map.transform.position, mapPos.transform.position, 1f * Time.deltaTime);
            //map.transform.rotation = Quaternion.LookRotation(cam.transform.position, map.transform.up);
            map.transform.localScale = Vector3.MoveTowards(map.transform.localScale, new Vector3(0.2923884f, 0.08427431f, 0.3694341f), 1f * Time.deltaTime);
        }
